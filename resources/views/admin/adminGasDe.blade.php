...
@section('user-gasDe', 'active')
@section('content')
{{--    <form action="{{ url('/userupdate' ,$item->id) }}" method="POST">--}}
    <form action="{{ url('/addgasname')}}" method="GET">
        {!! csrf_field() !!}
        <label class="progress-bar bg-info" role="progressbar">Add Gas</label>
...
<a href="{{ url('/gasDetailsRepo')}}" title="Report"> <button class="p-3 mb-2 bg-success text-white"> Generate Report</button></a>
@endsection
